...
UNIVERSE_CSV = Path("local/universe_151_200.csv")

def main():
    print("=" * 80)
    print("HARVESTER PIPELINE: US COMPANIES 151-200 (FY2017-2025) METHOD #1 PRIMARY RUN")
    ...
    manifest_path = OUTPUT_ROOT / "pipeline_151_200_pdf_manifest.csv"
    with RunLock(STATE_PATH):
        store = DiscoveryStore(STATE_PATH)
        try:
    ...
    pdf_downloaded = 0
    pdf_summary = {"downloaded": 0, "failed": 0, "bytes": 0, "pdfs_per_second": 0.0}
    ...
    # Count metrics for 151-200 filings
    total_pages_151_200 = 0
    total_bytes_151_200 = 0
    ledger = StateStore(STATE_PATH)
    tickers_151_200 = [c.ticker for c in companies]
    try:
        query = f"""
        SELECT r.relative_path, r.bytes, r.pages, r.sha256, c.ticker
        FROM reports r
        JOIN candidates cd ON r.pdf_url = cd.source_url
        JOIN companies c ON cd.company_key = c.company_key
        WHERE c.ticker IN ({','.join(f"'{t}'" for t in tickers_151_200)}) AND r.status='downloaded'
        """
        rows_151_200 = ledger.connection.execute(query).fetchall()
        for r_path, r_bytes, r_pages, r_sha, r_tick in rows_151_200:
            total_pages_151_200 += r_pages
            total_bytes_151_200 += r_bytes
    finally:
        ledger.close()

    print(f"\nFinal Audit Metrics for Ranks 151-200:")
    print(f"  Total Validated Reports : {len(rows_151_200)}")
    print(f"  Total Printed Pages     : {total_pages_151_200:,}")
    print(f"  Total Data Volume       : {total_bytes_151_200 / (1024*1024):.2f} MB")
    print(f"  Avg Pages / Report      : {total_pages_151_200 / max(len(rows_151_200), 1):.1f}")

    # Save summary json
    summary_data = {
        "universe": "US Ranks 151-200",
        ...
        "method1_direct_pdf": {
            ...
            "bytes_mb": round(pdf_summary.get("bytes", 0) / (1024*1024), 2),
            "elapsed_s": round(time.monotonic() - t_pdf0 if exported["pdf_rows"] > 0 else 0.0, 2),
        },
        "method2_chromium_fallback": {
            "rendered": render_summary["pdf_rendered"],
            "skipped": render_summary["skipped"],
            "failed": render_summary["failed"],
            "elapsed_s": render_summary["elapsed_s"],
            "docs_per_second": render_summary["documents_per_second"],
        },
        "verification": {
            "total_checked_store": v_summary["checked"],
            "ok": v_summary["ok"],
            "errors": len(v_summary["errors"]),
            "cohort_151_200_reports": len(rows_151_200),
            "cohort_151_200_pages": total_pages_151_200,
            "cohort_151_200_mb": round(total_bytes_151_200 / (1024*1024), 2),
        }
    }
    with open("local/pipeline_151_200_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary_data, f, indent=2)
    print("Saved pipeline_151_200_summary.json successfully.")
